...
if( !function_exists('add_twitter_og_image')){
	function add_twitter_og_image(){
		if( is_single() && get_theme_mod('malina_sharing_twitter',true) && !get_theme_mod('malina_seo_settings', false) ){
			global $post;
	...
	function malina_serch_title_highlights($title, $id){
		...
		if( !is_admin() && is_search() && $post instanceof WP_Post && ( $post_type === 'post' || $post_type === 'page' ) ){
			...
			$regEx = '\'(?!((<.*?)|(<a.*?)))(\b'. implode('|', $keys) . '\b)(?!(([^<>]*?)>)|([^>]*?</a>))\'iu';
      		$new_title = preg_replace($regEx, '<strong class="search-highlight">\0</strong>', $title);
			return $new_title;
		}
		return $title;
	}
...
if(!function_exists('malina_serch_excerpt_highlights')){
	function malina_serch_excerpt_highlights($post_excerpt){
		...
		if( !is_admin() && is_search() && $post instanceof WP_Post && ( $post_type === 'post' || $post_type === 'page' ) ){
			...
			$keys = explode(" ",$s);
			$regEx = '\'(?!((<.*?)|(<a.*?)))(\b'. implode('|', $keys) . '\b)(?!(([^<>]*?)>)|([^>]*?</a>))\'iu';
      		$post_excerpt = preg_replace($regEx, '<strong class="search-highlight">\0</strong>', $post_excerpt);
		}
		return $post_excerpt;
	}
...
function malina_remove_subscriber_from_list($emails){
	$subscribers_list = array();
	$subscribers_list = maybe_unserialize(get_option('malina_subscribe_mailing_list'));
	$emails = str_replace(' ', '', $emails);
	$subscribers = explode(',', $emails);
	$subscribers_list = array_diff($subscribers_list, $subscribers);
	update_option('malina_subscribe_mailing_list', serialize($subscribers_list));
	echo '<script>alert("'.esc_html__('You are now successfully unsubscribed', 'malina-elements').'");</script>';
	return true;
}
if ( isset($_POST['malina_remove_subscriber']) && $_POST['malina_remove_subscriber'] != ''){
	malina_remove_subscriber_from_list($_POST['malina_remove_subscriber']);
}
...
function malina_add_subscriber_info($subscriber, $post_url, $post_thumbnail, $post_title, $post_excerpt){
			...
			<table cellpadding=\"0\" cellspacing=\"0\" border=\"0\">
				<tr>
					<td style=\"border:1px solid #ededed;background-color:#fff;max-width:400px;margin:0 auto;border-radius:5px;padding:5px;\">
						...
						<a href=\"{$post_url}\" style=\"text-decoration:none;color:#d87b4d;\">".esc_html__('read more', 'malina-elements')."</a>
						</span>
					</td>
				</tr>
				<tr>
					<td>
						<form id=\"malina_unsubscribe_form\" action=\"".esc_url(home_url('/'))."\" method=\"POST\" target=\"_blank\" style=\"padding-top:10px;\"><input type=\"hidden\" id=\"malina_remove_subscriber\" name=\"malina_remove_subscriber\" value=\"{$subscriber}\" /><button type=\"submit\" style=\"border:0;outline:none;cursor:pointer;background-color:none;background:none;color:#a5a5a5;font-size:11px;\">".esc_html__('Unsubscribe', 'malina-elements')."</button></form>
					</td>
				</tr>
			</table>
		</body>
...
function malina_send_notification( $new_status, $old_status, $post ) {
    ...
    if( $new_status == 'publish' && $old_status != 'publish' ){
    	foreach ($subscribers_list as $subscriber) {
    		if( get_theme_mod('malina_newsletter_email_template', '') == '' ){ 
    			$message = malina_add_subscriber_info($subscriber, $post_url, $post_thumbnail, $post_title, $post_excerpt);
    		}
