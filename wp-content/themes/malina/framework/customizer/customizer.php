	...
	*/
	...
	if(class_exists('RevSlider')){
		...
		$wp_customize->add_control(
		    ...
		    array(
		        'section'   => 'malina_blog_options',
		        'label'     => esc_html__('Show posts carousel?','malina'),
		        'description'     => sprintf('<a href="#customize-control-malina_home_hero_posts">%s</a>', esc_html__('Options for carousel here','malina')),
		        'type'      => 'select',
		        'choices'	=> array('hide'=>esc_html__('Hide', 'malina'), 'show' => esc_html__('Show', 'malina'))
		    )
		);
	}
	/*
	...
	*/
	...
	if(function_exists('malina_get_social_links')) {
		...
		$wp_customize->add_control(
		    ...
		    array(
		        ...
		        'description' => esc_html__('If the previous method with username doesn"t work, please generate access token.','malina').' <a href="https://api.instagram.com/oauth/authorize?client_id=423965861585747&redirect_uri=https://connect.smashballoon.com/auth/ig/instagram-basic-display-redirect.php&response_type=code&scope=user_profile,user_media&state={%27{url=https://malina.artstudioworks.net/auth/admin.php?page=sb-instagram-feed,opt=out}%27}" target="__blank">'.esc_html__('Get access token', 'malina').'</a>',
		        'type'      => 'text',
		    )
		);
	...
	$wp_customize->add_control(
	    ...
	    array(
	        ...
	        'label'     => esc_html__('Enable webfontloader?','malina'),
	        'description'     => esc_html__('It helps you to optimize fonts loading.','malina'),
	        'type'      => 'checkbox'
	    )
	);
	$wp_customize->add_setting(
	...
	function malina_google_fonts() {
		...
		foreach($googlefonts as $getfonts) {
			if(!in_array($getfonts, $default)) {
				...
				$google_style_t = get_theme_mod('malina_google_font_'.$font_l, '400');
		...
		if( $fonts_set != '' ){
			if( get_theme_mod('malina_webfont_loader', false) ){
				$cnt = 1;
				$loadfonts = explode('|', $fonts_set);
				$tmp_fonts = '';
				$numItems = count($loadfonts);
				$swap = '';
				foreach ($loadfonts as $font) {
				...
				$webfontloader = "
					...
					(function(d) {
						...
						wf.src = '".get_template_directory_uri()."/js/webfont.js';
						wf.async = true;
						s.parentNode.insertBefore(wf, s);
					})(document);";
				wp_add_inline_script('jquery', $webfontloader);
			} else {
				wp_enqueue_style( 'google-fonts-malina', esc_url("//fonts.googleapis.com/css?family=".$fonts_set.$subset_str.'&display=swap'), array(), NULL, 'all' );
			}
		}
	}
}