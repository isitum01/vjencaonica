...
<div id="revslider-section" style="<?php echo esc_attr($style); ?>">
	...
	<?php
		if( get_theme_mod('malina_home_revslider_posts', 'hide') == 'show' ){
			$out = '';
						...
						while ( $the_query->have_posts() ) {
							...
							$out .= '<div class="recent-post-item">';
								if( has_post_thumbnail() ){
									$out .= '<div class="post-img"><a href="'.esc_url(get_the_permalink()).'" title="'.esc_attr__('Permalink to', 'malina').' '.esc_attr(the_title_attribute('echo=0')).'" rel="bookmark">'.get_the_post_thumbnail(get_the_ID(), $thumbsize).'</a></div>';
								}
