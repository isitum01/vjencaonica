...
<div id="revslider-section" style="<?php echo esc_attr($padding_css); ?>">
	...
	<?php
		if( rwmb_meta('malina_page_herosection_revslider_posts') == 'show' ){
			$out = '';
			...
			if( $posts_per_page != '0' ){
				...
				if( have_posts() ) {
					...
					$out .= '<div class="bg"><div id="recent-posts-slider" class="recent-posts">';
						while ( have_posts() ) {
							the_post();
							$out .= '<div class="recent-post-item">';
								if( has_post_thumbnail() ){
									$out .= '<div class="post-img"><a href="'.esc_url(get_the_permalink()).'" title="'.esc_attr__('Permalink to', 'malina').' '.esc_attr(the_title_attribute('echo=0')).'" rel="bookmark">'.get_the_post_thumbnail(get_the_ID(), $thumbsize, array('class' => 'blog-carousel-img')).'</a></div>';
								}
