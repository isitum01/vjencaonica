...
$sidebar = get_theme_mod('malina_sidebar_pos_search', 'sidebar-right');
if($sidebar != 'none'){
...
$excerpt_count = get_theme_mod('malina_search_excerpt_count', '15');
 
$s = get_search_query();
?>
<div id="page-wrap-blog" class="container">
	<div id="content" class="<?php echo esc_attr($sidebar_pos); ?>">
		<?php if(get_theme_mod( 'malina_search_display_page_title', false)){ ?>
		<header  class="page-title">
            <h2><?php printf( __( 'Search results for:', 'malina' ).' %s', '<span>' . $s . '</span>' ); ?></h1>
        </header>
        <?php } ?>
		<div class="row">
			<?php

			$columns = get_theme_mod( 'malina_search_columns', 'span6');
	      	$display_categories = true;
	      	$display_date = get_theme_mod( 'malina_display_post_date_search', 'true');
	      	$date_on_hover = $display_date == 'hover' ? ' date_on_hover' : '';
			$display_date = ($display_date == 'true' || $display_date=='1' || $display_date=='hover');
	      	$display_comments = get_theme_mod( 'malina_display_post_comments_search', false);
	      	$show_readmore = get_theme_mod( 'malina_display_read_more_search', false);
	      	if( function_exists('MalinaGetPostViews')){
	      		$display_views = get_theme_mod( 'malina_display_post_views_search', true);
	      	} else {
	      		$display_views = false;
	      	}
	      	if( function_exists('getPostLikeLink') ){
	      		$display_likes = get_theme_mod( 'malina_display_post_likes_search', true);
	      	} else {
	      		$display_likes = false;
	      	}
	      	if( function_exists('malina_calculate_reading_time') ){
	      		$display_read_time = get_theme_mod( 'malina_display_post_read_time_search', true);
	      	} else {
	      		$display_read_time = false;
	      	}
	      	$pagination = get_theme_mod( 'malina_display_post_pagination_search', 'standard');
	      	$display_readmore = 'true';
	      	$ignore_featured = $ignore_sticky_posts = true;
	      	$text_align = 'textcenter';
			$out = '';
			$post_style = get_theme_mod('malina_search_style', 'style_1');
			$thumbsize = get_theme_mod( 'malina_thumbsize_search', 'medium');
			...
			if(($post_style == 'style_1' || $post_style == 'style_4' || $post_style == 'style_8' || $post_style == 'style_10' ) || $pagination == 'true' || $pagination == 'infinitescroll' ){
				...
				if( $pagination != 'infinitescroll' ){
			    	$script .= "jQuery(window).on('load', function(){ jQuery(window).unbind('.infscr'); });";
			    }
			...
			echo ''.$out;
				...
				<div class="span12">
					...
					<div class="text mb30">
						<?php esc_html_e('Sorry, but nothing matched your search terms. Please try again with some different keywords.', 'malina'); ?>
					</div>
				</div>
				<div class="span8"><?php get_search_form(); ?></div>
