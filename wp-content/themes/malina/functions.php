	...
	function malina_register_required_plugins() {
		$plugins = array(
	        ...
	        array(
	        	...
	        	'slug'      => 'wp-megamenu',
	        	'source'   			=> 'https://www.artstudioworks.net/recommended-plugins/wp-megamenu.zip',
				'required' 	=> false, 
	        ),
	        array(
...
if(!function_exists('malina_import_files')){
	function malina_import_files() {
	    return array(
	        ...
	        array(
	            'import_file_name'           => 'Moona',
	            'local_import_file'            => trailingslashit( get_template_directory() ).'framework/demos/elementor/moona/demo.xml',
	            'local_import_customizer_file' => trailingslashit( get_template_directory() ).'framework/demos/elementor/moona/customizer.dat',
	            'import_preview_image_url'   => trailingslashit( get_template_directory_uri() ).'framework/demos/elementor/moona/demo.jpg',
	            'preview_url'                => 'https://www.malina.artstudioworks.net/adventures',
	            'categories' => ['Elementor Pro']
	        ),
	        array(
	            'import_file_name'           => 'Moncler',
	            'local_import_file'            => trailingslashit( get_template_directory() ).'framework/demos/elementor/moncler/demo.xml',
	            'local_import_customizer_file' => trailingslashit( get_template_directory() ).'framework/demos/elementor/moncler/customizer.dat',
	            'import_preview_image_url'   => trailingslashit( get_template_directory_uri() ).'framework/demos/elementor/moncler/demo.jpg',
	            'preview_url'                => 'https://www.malina.artstudioworks.net/moncler',
	            'categories' => ['Elementor Pro']
	        ),
	        array(
	            'import_file_name'           => 'Cosmo Demo',
...
function malina_check_plugins_before_import($selected_import){
	...
	if( (strpos($selected_import['local_import_file'], 'moncler') !== false || strpos($selected_import['local_import_file'], 'moona') !== false) && !defined('ELEMENTOR_PRO_VERSION') ){
		$url = 'https://elementor.com/pro/';
		$response['message'] = sprintf(
			__( '%1$sPlease install %2$sElementor Pro Plugin%3$s, before this demo importing. %4$sThis premium plugin are not included.%5$s', 'malina' ),
			'<div class="notice  notice-warning"><p>',
...
function malina_menu_logo_center( $items, $args ) {
    ...
    if( get_theme_mod('malina_header_search_button', false) && $args->theme_location == 'main_navigation' ) {
		$search_button = '<li class="search-link"><a href="javascript:void(0);" class="search-button"><i class="fas fa-search"></i></a><li>';
...
function malina_menu_item_search( $items, $args ){
	$logo_item = '';
	if ( $args->theme_location == 'main_navigation' ) {
        $logo_item = '<li class="menu-item-search"><div class="search-link"><a href="javascript:void(0);" class="search-button"><i class="fas fa-search"></i></a></div></li>';
    }
    $items .= $logo_item;
    return $items;
}
...
if(!function_exists('malina_single_post_gallery')){
	function malina_single_post_gallery($postID = false, $echo = true){
		...
		$images = rwmb_get_value( 'malina_gallery_images', array( 'size' => $img_size ) );
		...
		if ( !empty($images) ){
			switch ($gallery_type) {
				case 'slideshow_2':
					$owl_custom = 'jQuery(window).on(\'load\', function(){
...
if( !function_exists('malina_get_post_format_content') ){
	function malina_get_post_format_content($echo, $img_size){
		if($img_size == 'disable'){
			return '';
		}
		global $_wp_additional_image_sizes;
